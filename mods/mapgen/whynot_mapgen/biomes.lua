...
local function copy_biome(old_name, variant_suffix, def)
...
for name, def in pairs(minetest.registered_biomes) do
    ...
    if (string.find("coniferous_forest deciduous_forest savanna rainforest taiga desert grassland", def.name)) then
        local floatlands_biome = copy_biome(def.name, "floatlands", def)
        floatlands_biome.y_max = floatlands_biomes_ymax - 1
        floatlands_biome.y_min = lower_atmosphere_biome_ymax

        -- Make all floatlands dungeons princess-style
        def.node_dungeon = "princess:princess_tower_crack_brick"
        def.node_dungeon_alt = "princess:princess_mossy_dungeon_brick"
        def.node_dungeon_stair = "stairs:stair_princess_mossy_dungeon_brick"
